...
try{
  ...
  function bindFilePicker(which) {
    var input = document.getElementById(which === 1 ? 'f1' : 'f2');
    if (input) input.addEventListener('change', function () { onFilePicked(which, input); });
  }
  document.querySelectorAll('[data-vision-file]').forEach(function (button) {
    button.addEventListener('click', function () { pickFile(Number(button.getAttribute('data-vision-file'))); });
  });
  document.getElementById('d1').addEventListener('click', function () { pickFile(1); });
  document.getElementById('d2').addEventListener('click', function () { pickFile(2); });
  [1, 2].forEach(function (which) {
    var drop = document.getElementById('d' + which);
