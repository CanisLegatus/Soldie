...
(function () {
  "use strict";
  // Keep the component document separate from the page document used for bootstrapping.
  var host = window.document.getElementById('visionHost');
  ...
  var visionDocument = root;

function $(id){return visionDocument.getElementById(id);}
function vStatus(msg,isErr){
  var el=visionDocument.getElementById('statusline');
  if(el){el.textContent=msg;el.style.color=isErr?'#D92D20':'#667085';}
}
function showBusy(on,text){
  var b=visionDocument.getElementById('busy');
  if(!b){return;}
  if(text){var t=visionDocument.getElementById('busyT');if(t){t.textContent=text;}}
  if(on){b.classList.add('on');}else{b.classList.remove('on');}
}
function pickFile(w){
  var inp=visionDocument.getElementById(w===1?'f1':'f2');
  if(inp){inp.click();}
}
...
function initApp(){
  ...
  visionDocument.addEventListener('keydown',function(e){
    if(e.key==='Escape'){closeDrill();closeCustom();}
  });
  visionDocument.addEventListener('click',function(e){
    var t=e.target;
    while(t&&t!==visionDocument){
